...
rule mutect_tumoronly:
    ...
    output:
        ...
        fir=resolve_single_filepath(config.get("paths").get("workdir"),"results/tumoronly/{sample}_tumor_normal_f1r2.tar.gz"),
        stats=resolve_single_filepath(config.get("paths").get("workdir"),"results/tumoronly/{sample}_somatic.vcf.gz.stats")
    params:
        custom=java_params(tmp_dir=config.get("paths").get("tmp_dir"), multiply_by=5),
...
rule orientation_model_tumoronly:
    ...
    output:
        resolve_single_filepath(config.get("paths").get("workdir"),"results/filters/tumoronly/{sample}_read-orientation-model.tar.gz")
    params:
        custom=java_params(tmp_dir=config.get("paths").get("tmp_dir"), multiply_by=5),
...
rule calculate_contamination_tumoronly:
    ...
    output:
        table=resolve_single_filepath(config.get("paths").get("workdir"),"results/filters/tumoronly/{sample}_contamination.table"),
        segment=resolve_single_filepath(config.get("paths").get("workdir"),"results/filters/tumoronly/{sample}_tumor.segment")
    params:
        custom=java_params(tmp_dir=config.get("paths").get("tmp_dir"),multiply_by=5)
    log:
