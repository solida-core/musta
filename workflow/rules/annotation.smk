rule Funcotator:
    ...
    output:
        vcf=report(
            resolve_results_filepath(config.get("paths").get("results_dir"),"results/annotation/funcotator/{sample}_funcotated.maf"),
    ...
    log:
        resolve_results_filepath(config.get("paths").get("results_dir"),"logs/gatk/Funcotator/{sample}.funcotator.log")
    conda:
