rule filter_mutect:
    ...
    output:
        vcf=resolve_results_filepath(config.get("paths").get("results_dir"),"results/matched/{sample}_somatic_filtered.vcf.gz")
    params:
    ...
    log:
        resolve_results_filepath(config.get("paths").get("results_dir"),"logs/gatk/Mutect2/{sample}.filter_info.log")
    conda:
...
rule filter_mutect_tumoronly:
    ...
    output:
        vcf=resolve_results_filepath(config.get("paths").get("results_dir"),"results/tumoronly/{sample}_somatic_filtered.vcf.gz")
    params:
        custom=java_params(tmp_dir=config.get("paths").get("tmp_dir"), multiply_by=5),
        genome=config.get("resources").get("reference"),
        intervals=resolve_single_filepath(config.get("paths").get("workdir"),resolve_single_filepath("resources",config.get("resources").get("bed")))
...
rule gatk_SelectVariants:
    ...
    output:
        vcf=resolve_results_filepath(config.get("paths").get("results_dir"),"results/{sample}_somatic_filtered_selected.vcf.gz")
    params:
        custom=java_params(tmp_dir=config.get("paths").get("tmp_dir"),multiply_by=5),
        genome=config.get("resources").get("reference"),
        arguments=config.get("params").get("gatk").get("SelectVariants")
    log:
        resolve_results_filepath(config.get("paths").get("results_dir"),"logs/gatk/SelectVariants/{sample}.SelectVariants.log")
    conda:
        resolve_single_filepath(config.get("paths").get("workdir"),"workflow/envs/gatk.yaml")
    threads: conservative_cpu_count(reserve_cores=2, max_cores=99)
    resources:
