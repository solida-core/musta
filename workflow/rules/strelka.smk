...
rule strelka_out:
    ...
    threads: conservative_cpu_count(reserve_cores=2,max_cores=99),
    shell:
        "gunzip -c {input.snvs} | sed '0,/NORMAL/! s/NORMAL/{params.normal_name}/' | sed '0,/TUMOR/! s/TUMOR/{params.tumor_name}/' | bgzip -c > {output.snvs} ; "
        "gunzip -c {input.indels} | sed '0,/NORMAL/! s/NORMAL/{params.normal_name}/' | sed '0,/TUMOR/! s/TUMOR/{params.tumor_name}/' | bgzip -c > {output.indels} "
